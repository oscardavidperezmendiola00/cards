...
type Profile = {
  ...
  whatsapp?: string; email?: string; phone?: string; socials_json?: Socials; theme?: Theme;
};
...
export default function Perfil({ profile }: { profile: Profile | null }) {
  ...
  const theme: Theme = profile.theme ?? { primary: '#111827', accent: '#10b981', bg: '#0b0f19', text: '#e5e7eb' };
...
export const getServerSideProps: GetServerSideProps<{ profile: Profile | null }> = async (ctx) => {
  ...
  try {
    const xf = ctx.req.headers['x-forwarded-proto'];
